# ORBITIDE — Competitive UX Research
...
**ORBITIDE decision:** Never show a forced interstitial immediately after failure. Failure screens may offer voluntary rewarded help.
...
**ORBITIDE decision:** Offer optional rewarded placements such as Hint, temporary Extra Slot, or a small coin grant. The value exchange must be explicit before playback.
...
**ORBITIDE decision:** Boosters reduce friction; they never make an otherwise impossible campaign level possible. All shipped campaign levels must be solver-verified with standard rules and zero boosters.
...
**ORBITIDE decision:** Hard and Super Hard levels increase sequencing pressure and tempting wrong paths, not arbitrary luck. Mystery mechanics, if used, must preserve fair decision-making.
...
**ORBITIDE decision:** A regular player must be able to earn useful boosters through normal play. Purchases accelerate progress; they do not ransom it.
...
**ORBITIDE decision:** Save active puzzle state locally for exact resume. Cross-device cloud restoration of progress, wallet, and inventory is a 1.0 requirement. Durable purchase ownership is restored through RevenueCat/StoreKit.
...
**ORBITIDE decision:** Core campaign gameplay and level data remain local/offline-first. Backend sync must never sit in the move-resolution path.
...
**ORBITIDE decision:** Color is never the only meaningful signal. Explore subtle symbols/patterns and a Color Assist setting while retaining the premium visual language.
...
**ORBITIDE decision:** Motion polish has a performance budget. Cap particles, avoid runaway animation loops, suspend unnecessary rendering in the background, and test sustained sessions on real devices.
...
**ORBITIDE decision:** Teach through level design first, but provide a lightweight persistent How It Works / Boosters reference once multiple mechanics exist.
...
**ORBITIDE decision:** Use themed 10-level sets and larger worlds so future content drops have clear identity. Do not sacrifice level quality merely to increase the count.
...
5. After launch, prioritize ORBITIDE reviews, support feedback, and telemetry over competitor feedback.
