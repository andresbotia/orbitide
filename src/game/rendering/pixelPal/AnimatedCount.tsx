 ...
 * pop and costs no React commit per hit. Each digit is a 0–9 strip in a clipped
 * window, positioned by an animated translate — no animated text props.
 */
export const AnimatedCount = memo(function AnimatedCount({ palSize, pass, clock, active = false }: {
  ...
  const windowH = (numeral.lineHeight as number) + (numeral.textShadowColor ? WINDOW_PAD * 2 : 2);
  const pitch = Math.round(fontSize * STRIP_PITCH);
  const digitW = Math.ceil(fontSize * 0.66);
  const columns = String(Math.max(0, pass.charge.capacity)).length;
  const count = useDerivedValue(() => capacityAt(pass, clock.value));

  // Fewer digits than columns: shift the row so the visible digits stay centred.
...
function DigitColumn({ place, count, width, pitch, windowH, numeral }: {
  ...
  pitch: number;
  windowH: number;
  numeral: ReturnType<typeof palBadgeNumeralStyle>;
}) {
