...
test.each(WORLD_4)(
  'Curio Cabinet level $id solves and its witness replays through runtime',
  (level) => {
...
test('Curio Cabinet analyzer report stays scoped to Levels 31-40', async () => {
  ...
  expect(rows[0]!.solve.failPathLength).toBeNull();
  expect(rows[1]!.solve.failPathLength).toBeNull();
  expect(rows[2]!.solve.failPathLength).toBeGreaterThan(2);
  ...
  for (const row of rows.slice(3, 6)) {
    expect(row).toMatchObject({ authored: 'medium', suggested: 'medium' });
    expect(row.solve.length).toBeGreaterThanOrEqual(8);
    expect(row.solve.minWinningPeak).toBeGreaterThanOrEqual(1);
    expect(row.solve.heldLaunches).toBeGreaterThanOrEqual(1);
    expect(row.solve.maxHolding).toBe(3);
  }
  expect(rows[3]!.solve.failPathLength).toBeNull();
  expect(rows[4]!.solve.heldLaunches).toBeGreaterThan(rows[3]!.solve.heldLaunches);
  ...
  expect(rows[6]!.solve.minWinningPeak).toBe(2);
  expect(rows[7]!.solve.length).toBeGreaterThanOrEqual(10);
  expect(rows[7]!.solve.heldLaunches).toBeGreaterThanOrEqual(3);
  ...
  expect(rows[8]!.solve.lossProbability).toBeGreaterThan(0.5);
  ...
  expect(rows[9]!.solve.lossProbability).toBeLessThan(0.5);
