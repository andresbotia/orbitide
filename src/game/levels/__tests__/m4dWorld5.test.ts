...
test.each(WORLD_5)(
  'Prism Works level $id solves and its witness replays through runtime',
  (level) => {
    const result = solve(level);
...
test('Prism Works analyzer report stays scoped to Levels 41-50', async () => {
  ...
  expect(rows.slice(0, 3).map((row) => row.solve.length)).toEqual([6, 7, 9]);
  expect(rows.slice(0, 3).map((row) => row.solve.heldLaunches)).toEqual([2, 2, 3]);
  ...
  expect(rows[5]!.solve).toMatchObject({ length: 9, heldLaunches: 4 });
  expect(rows[6]!.solve).toMatchObject({ length: 10, heldLaunches: 3 });
  expect(rows[7]!.solve).toMatchObject({ length: 10, heldLaunches: 4 });
  ...
  expect(rows[8]!.solve.failPathLength).toBeGreaterThan(2);
  expect(rows[9]!.solve.failPathLength).toBeNull();
  expect(rows[9]!.solve.minWinningPeak).toBe(2);
